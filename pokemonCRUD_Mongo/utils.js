...
let get_card_handler = (_) => {
    ...
    axios
        ...
        .then(resp => {
            ...
            document.querySelector('#pokemon-list').appendChild(make_card_elem(resp.data));
    ...
    axios
        ...
        .then(resp => {
            ...
            for (const card in resp.data) {
                document.querySelector('#pokemon-list').appendChild(make_card_elem(resp.data[card]));
                let new_item = document.querySelector('#pokemon-list').lastElementChild;
                new_item.querySelector('.remove-item').addEventListener('click', (event) => remove_item(new_item));
            }
...
function make_card_elem(data) {

    let template = document.createElement('div');
    template.className = "text-white bg-secondary m-1";
        ...
        case "pokemon":
            ...
            template.innerHTML =   `
                                    ...
                                    <div class=\"card-body \">
                                        ...
                                        <div class='flex-break'></div>
                                        <button class=\"remove-item btn btn-primary float-right mb-3\">remove</button>
                                    </div>
                                   `;
            break;
        default:
            template.innerHTML = `
                                <div class=\"card-header\">
                                    ...
                                    <div class=\"card-body\">
                                    <div class='flex-break'></div>
                                    <h5>Type: ${data.cardType[0].toUpperCase() + data.type.substring(1)}</h5>
                                    <p>${data.details}</p>
                                    <div class='flex-break'></div>
                                    <button class=\"remove-item btn btn-primary float-right mb-3\">remove</button>
                                </div>
